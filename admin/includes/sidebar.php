<aside id="sidebar" class="sidebar">
      <ul class="sidebar-nav" id="sidebar-nav">
        <li class="nav-item">
          <a class="nav-link collapsed" href="dashboard.php">
            <i class="bi bi-grid"></i>
            <span>Dashboard</span>
          </a>
        </li>
        <!-- End Dashboard Nav -->
        <li class="nav-item">
          <a
            class="nav-link collapsed"
            data-bs-target="#kegiatan-nav"
            data-bs-toggle="collapse"
            href="#"
          >
            <i class="bi bi-menu-button-wide"></i><span>Kegiatan Terbaru</span
            ><i class="bi bi-chevron-down ms-auto"></i>
          </a>
          <ul
            id="kegiatan-nav"
            class="nav-content collapse"
            data-bs-parent="#sidebar-nav"
          >
            <li>
              <a href="manage-post.php">
                <i class="bi bi-pencil"></i><span>Manage Post</span>
              </a>
            </li>
            <li>
              <a href="post-trash.php">
                <i class="bi bi-trash"></i><span>Trash</span>
              </a>
            </li>
          </ul>
        </li>
        <!-- End Kegiatan Nav -->
        <li class="nav-item">
          <a
            class="nav-link collapsed"
            data-bs-target="#pengumuman-nav"
            data-bs-toggle="collapse"
            href="#"
          >
            <i class="bi bi-menu-button-wide"></i><span>Pengumuman</span
            ><i class="bi bi-chevron-down ms-auto"></i>
          </a>
          <ul
            id="pengumuman-nav"
            class="nav-content collapse"
            data-bs-parent="#sidebar-nav"
          >
            <li>
              <a href="manage-announcement.php">
                <i class="bi bi-pencil"></i><span>Manage Post</span>
              </a>
            </li>
            <li>
              <a href="announcement-trash.php">
                <i class="bi bi-trash"></i><span>Trash</span>
              </a>
            </li>
          </ul>
        </li>
        <!-- End Pengumuman Nav -->
        <li class="nav-item">
          <a class="nav-link collapsed" href="struktur-organisasi.php">
            <i class="bi bi-grid"></i>
            <span>Struktur Organisasi</span>
          </a>
        </li>
        <!-- End struktur Nav -->
        <li class="nav-item">
          <a class="nav-link collapsed" href="contact.php">
            <i class="bi bi-telephone"></i>
            <span>Kontak</span>
          </a>
        </li>
        <!-- End struktur Nav -->



      </ul>
    </aside>